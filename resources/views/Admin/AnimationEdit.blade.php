...
@section('content')
    <div class="row px-3">
        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb" class="">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page"> Edit Animation </li>
            </ol>
        </nav>
    ...
    <div class="row ml mt-3">
        <div>
            <h4 class=" mb-0  ml-3 blod">Edit Animation</h4>
        </div>
    </div>
    <div class="container mt-5 input-add">
        <form action="{{route('Update_Animation',$item->id)}}" method="POST">
            @csrf
            @method('PUT')
            ...
            @enderror
            <input type="text" name="slug" placeholder="Enter-Slug" class="mt-5" id="convert_slug" value="{{$item->slug}}">
            <br>
            @error('slug')
                <p class="validation-thongbao">{{ $message }}</p>
            @enderror
