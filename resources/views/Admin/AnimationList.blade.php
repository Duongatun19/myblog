    ...
    <div class="row px-3">
        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb" class="">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">List Animation</li>
            </ol>
        </nav>
    </div>
    <div class="row header-admin">
        <div>
            <h1 class="h3 mb-0 text-gray-800 ml-3">List Animation</h1>
        </div>
        <div>
            <a href="{{route('PageAddAnimation')}}"><Button class="btn btn-primary">ADD</Button></a>
        </div>
    </div>
    <div class="row mt-5">
        <table class="table table-sm mx-5" id="dataTables-example">
            ...
            @foreach ($animations as $item)
                <tbody>
                    ...
                    <td>{{ $item->slug }}</td>
                    <td>
                        <input type="checkbox" class="toggle-animation" data-id="{{ $item->id }}" data-toggle="toggle"
                        data-style="slow" data-on="Enabled"
                        data-off="Disabled"{{ $item->status == true ? 'checked' : '' }}>
                    </td>
                    <td>
                        <div class="controll-item">
                            <form action="{{ route('delete_animation', $item->id) }}" method="post">
                                @csrf
                                @method('DELETE')
                                <button class="control-delete"><i class="fa-solid fa-trash-can"></i></button>&nbsp;&nbsp;
                            </form>
                            <form action="{{ route('edit_animation', $item->id) }}">
